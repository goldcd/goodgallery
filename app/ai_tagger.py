...
class AITagger:
    ...
    def load_model(self):
        ...
        try:
            ...
            # Use AutoModelForImageTextToText (formerly AutoModelForVision2Seq)
    ...
    def generate_text_response(self, image: Image.Image, prompt: str) -> str:
        """
        Generate a text response using the VLM as an LLM.
        This allows us to use the visual model for text reasoning tasks 
        by passing a dummy image and a text-heavy prompt.
        """
        if not self.is_loaded:
            raise RuntimeError("Model not loaded")
        ...
        try:
            # Qwen Handling
            if self.is_qwen:
                # Clean prompt logic similar to batch
                clean_prompt = prompt
                clean_prompt = re.sub(r'USER:\s*<image>\s*', '', clean_prompt, flags=re.IGNORECASE).strip()
                if "ASSISTANT:" in clean_prompt:
                    clean_prompt = clean_prompt.split("ASSISTANT:")[0].strip()
                ...
                with torch.no_grad():
                    ...
                    generated_ids = self.model.generate(
                        **inputs, 
                        max_new_tokens=1024,
                        do_sample=False  # Deterministic, faster, less prone to loops
                    )
            ...
            # LLaVA Handling
                ...
                generate_ids = self.model.generate(
                    **inputs,
                    max_new_tokens=1024,
                    do_sample=False # Deterministic for logic
                )
